...
import { ResponsiveAd } from 'src/components/AdSense';
...
export default function LoanCalculator() {
  ...
  const [loanType, setLoanType] = useState<'mortgage' | 'jeonse' | 'credit'>('mortgage');
  const [region, setRegion] = useState<'regulated' | 'non-regulated'>('regulated');
  const [isFirstTime, setIsFirstTime] = useState<boolean>(true);
  const [hasMultipleProperties, setHasMultipleProperties] = useState<boolean>(false);
  ...
  const getLoanPolicy = () => {
    ...
    let dsrRatio = 0.4; // 기본 DSR 40%
    
    if (region === 'regulated') {
      // 규제지역 (서울, 수도권)
      if (price >= 1500000000) { // 15억 초과
        ltvRatio = 0.4;
        dtiRatio = 0.3;
      } else if (price >= 900000000) { // 9억~15억
        ltvRatio = 0.5;
        dtiRatio = 0.4;
      } else { // 9억 이하
        ltvRatio = 0.6;
  ...
  const calculateLoan = () => {
    ...
    const existingPayment = parseFloat(existingMonthlyPayment.replace(/,/g, '')) || 0;
    const term = parseInt(loanTerm);
    const customRate = parseFloat(interestRate) || 0;
    ...
    const dtiAmount = (income * policy.dtiRatio) / ((rate / 12) * Math.pow(1 + rate / 12, term * 12) / (Math.pow(1 + rate / 12, term * 12) - 1)); // DTI 기준
    const maxLoanAmount = Math.min(ltvAmount, dtiAmount);

    // 월 상환액 계산
    const monthlyRate = rate / 12;
    const totalMonths = term * 12;
    const principalInterest = maxLoanAmount * (monthlyRate * Math.pow(1 + monthlyRate, totalMonths)) / (Math.pow(1 + monthlyRate, totalMonths) - 1);
    ...
    const totalRequiredFunds = downPayment + intermediatePayment + balance + purchaseCosts;
    ...
    let recommendation = '';
    if (dsrRatio <= 0.3) {
      recommendation = '매우 안전한 수준입니다. 대출 승인 가능성이 높고, 여유 있는 상환이 가능합니다.';
    } else if (dsrRatio <= 0.4) {
      recommendation = '적정 수준입니다. 대출 승인 가능성이 있으나, 가계 지출 관리가 필요합니다.';
    } else if (dsrRatio <= 0.6) {
      recommendation = '위험 수준입니다. 대출 승인이 어려울 수 있으며, 소득 증대나 기존 부채 정리가 필요합니다.';
    } else {
      recommendation = '매우 위험한 수준입니다. 현재 조건으로는 대출이 매우 어렵습니다.';
    }
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100">
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold text-gray-900 mb-4">🏦 종합 대출 계산기</h1>
          <p className="text-lg text-gray-600">
            주택담보대출, 전세대출, 필요자금을 종합적으로 계산하여 최적의 대출 계획을 세우세요
          </p>
        </div>
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white rounded-xl shadow-lg p-6">
              <h2 className="text-xl font-semibold text-gray-900 mb-4">🏠 부동산 정보</h2>
              ...
              <div className="grid md:grid-cols-2 gap-4 mb-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      setPropertyPrice(value.replace(/\B(?=(\d{3})+(?!\d))/g, ','));
                    }}
                ...
                <div>
                  ...
                  <select
                    value={loanType}
                    onChange={(e) => setLoanType(e.target.value as 'mortgage' | 'jeonse' | 'credit')}
                    className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                  >
              ...
              <div className="grid md:grid-cols-2 gap-4">
                <div>
                  ...
                  <select
                    value={region}
                    onChange={(e) => setRegion(e.target.value as 'regulated' | 'non-regulated')}
                    className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                  >
                ...
                <div className="flex items-center space-x-4 pt-8">
                  ...
                  <label className="flex items-center">
                    <input
                      ...
                      onChange={(e) => setHasMultipleProperties(e.target.checked)}
                      className="rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                    />
            ...
            {/* 소득 및 부채 정보 */}
            <div className="bg-white rounded-xl shadow-lg p-6">
              <h2 className="text-xl font-semibold text-gray-900 mb-4">💰 소득 및 부채 정보</h2>
            ...
            {/* 대출 조건 */}
            <div className="bg-white rounded-xl shadow-lg p-6">
              <h2 className="text-xl font-semibold text-gray-900 mb-4">⚙️ 대출 조건</h2>
          ...
          <div className="space-y-6">
            {result ? (
              <>
                {/* DSR 현황 */}
                <div className="bg-white rounded-xl shadow-lg p-6">
                  <h3 className="text-lg font-semibold text-gray-900 mb-4">📊 DSR 현황</h3>
                  <div className={`rounded-lg p-4 text-center ${result.isEligible ? 'bg-green-50' : 'bg-red-50'}`}>
                    <div className={`text-2xl font-bold mb-2 ${result.isEligible ? 'text-green-600' : 'text-red-600'}`}>
                      {result.dsrRatio.toFixed(1)}%
                    </div>
                    <div className="text-sm text-gray-600 mb-2">
                      한도: {result.dsrLimit}%
                    </div>
                    <div className={`font-medium ${result.isEligible ? 'text-green-700' : 'text-red-700'}`}>
                ...
                <div className="bg-white rounded-xl shadow-lg p-6">
                  ...
                  <div className="space-y-3">
                    <div className="flex justify-between">
                      ...
                      <span className="font-semibold text-blue-600">{formatWon(result.maxLoanAmount)}</span>
                    </div>
                  </div>
                </div>
                ...
                {/* 월 납입액 */}
                <div className="bg-white rounded-xl shadow-lg p-6">
                  <h3 className="text-lg font-semibold text-gray-900 mb-4">📅 월 납입액</h3>
                  <div className="space-y-3">
                    <div className="flex justify-between">
                      <span className="text-gray-600">원리금</span>
                      <span className="font-medium">{formatNumber(result.principalInterest)}원</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-600">보험료</span>
                      <span className="font-medium">{formatNumber(result.insuranceAmount)}원</span>
                    </div>
                    <div className="border-t pt-3">
                      <div className="flex justify-between">
                        <span className="font-semibold text-gray-900">총 월 납입액</span>
                        <span className="font-bold text-lg text-blue-600">{formatNumber(result.totalMonthlyPayment)}원</span>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                {/* 필요 자금 */}
                <div className="bg-white rounded-xl shadow-lg p-6">
                  <h3 className="text-lg font-semibold text-gray-900 mb-4">💸 필요 자금</h3>
                  <div className="space-y-3">
                    <div className="flex justify-between">
                      <span className="text-gray-600">계약금 (10%)</span>
                      <span className="font-medium">{formatWon(result.downPayment)}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-600">중도금 (10%)</span>
                      <span className="font-medium">{formatWon(result.intermediatePayment)}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-600">잔금</span>
                      <span className="font-medium">{formatWon(result.balance)}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-600">취득비용</span>
                      <span className="font-medium">{formatWon(result.purchaseCosts)}</span>
                    </div>
                    <div className="border-t pt-3">
                      <div className="flex justify-between">
                        <span className="font-semibold text-gray-900">총 필요자금</span>
                        <span className="font-bold text-lg text-red-600">{formatWon(result.totalRequiredFunds)}</span>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                {/* 총 상환 정보 */}
                <div className="bg-white rounded-xl shadow-lg p-6">
                  <h3 className="text-lg font-semibold text-gray-900 mb-4">📈 총 상환 정보</h3>
                  <div className="space-y-3">
                    <div className="flex justify-between">
                    ...
                    <div className="border-t pt-3">
                      <div className="flex justify-between">
                        ...
                        <span className="font-bold text-lg text-gray-900">{formatWon(result.totalAmount)}</span>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-blue-50 rounded-xl border border-blue-200 p-6">
                  <h3 className="text-lg font-semibold text-blue-900 mb-3">💡 추천사항</h3>
              ...
              <div className="bg-white rounded-xl shadow-lg p-8">
                <div className="text-center text-gray-500 py-8">
                  ...
                  <p>부동산 정보와 소득을 입력하고<br />계산 버튼을 눌러주세요</p>
                </div>
              </div>
            )}
          </div>
        ...
        <div className="mt-8 bg-yellow-50 border border-yellow-200 rounded-xl p-8">
          <h3 className="text-xl font-semibold text-yellow-800 mb-4">📋 2024년 대출 정책 참고사항</h3>
          ...
          <div className="mt-4 p-4 bg-yellow-100 rounded-lg">
            <p className="text-sm text-yellow-800 font-medium">
              ⚠️ 본 계산기는 참고용이며, 실제 대출 승인 및 조건은 금융기관의 심사를 통해 결정됩니다.
              정확한 대출 상담은 각 금융기관에 문의하시기 바랍니다.
            </p>
          </div>
        </div>
      </main>
