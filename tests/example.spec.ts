...
test('homepage has title and navigation', async ({ page }) => {
  ...
  // 메인 헤더 확인
  await expect(page.getByRole('heading', { name: '부동산 계산기' })).toBeVisible();
...
test('navigation to purchase cost calculator', async ({ page }) => {
  ...
  // 페이지 제목 확인
  await expect(page.getByRole('heading', { name: '매입 비용 계산기' })).toBeVisible();
});
...
test('map section is visible on homepage', async ({ page }) => {
  ...
  await expect(page.getByText('지도에서 원하는 지역의 부동산 위치를 확인하고')).toBeVisible();
});